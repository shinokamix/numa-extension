...
pnpm dev              # start the development watcher
pnpm dev:firefox      # start the Firefox development watcher
pnpm check            # formatting, code, architecture, CSS, dead code, types, and tests
...
- `src/entrypoints/` — WXT entrypoints for the popup and options page
- `src/entrypoints/options/router/routeTree.gen.ts` — generated by TanStack Router; never edit it manually
...
- Treat `package.json` scripts and checked-in tool configuration as authoritative. Do not assume which formatter or linter the project uses.
- Use Prettier for formatting, ESLint for JavaScript and TypeScript, Stylelint for CSS, Steiger for architecture rules, and Knip for dead-code checks.
