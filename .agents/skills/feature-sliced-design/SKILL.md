...
description: Use for structural frontend work involving Feature-Sliced Design layers, code placement, public APIs, import boundaries, cross-imports, migrations, or WXT/FSD integration.
---
...
This profile takes precedence over general upstream examples. Read `../../../docs/architecture.md` before structural work; it is Numa's authoritative architecture policy. WXT owns `src/entrypoints`, which fulfill App-layer responsibilities as composition roots, so do not create `src/app`. Entrypoints may compose public APIs from every lower FSD layer, while lower layers must not import entrypoints. Keep Steiger on its recommended FSD configuration, use the repository's existing ESLint boundary rules, and run `pnpm lint:arch` plus the checks required by the root `AGENTS.md`.
