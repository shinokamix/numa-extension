---
name: reactuse
description: Use when Numa needs a generic reusable React hook and ReactUse may provide suitable source; not for feature-specific orchestration or general React work.
license: MIT
metadata:
...
## Numa project profile

This profile takes precedence over general upstream guidance. Read `../../../DESIGN.md`; it is Numa's authoritative hook-sourcing policy. Use this skill only for an immediate generic hook need. Manually copy the minimum suitable ReactUse source into its current FSD owner, record the pinned source URL and numbered adaptations, and treat the result as project-owned code. Do not install ReactUse, another dependency, or a registry/generator unless the user explicitly requests it and the repository's dependency-safety rules are satisfied. Keep feature orchestration with its feature, promote a hook to Shared only after independent reuse, and run the checks required by the root `AGENTS.md`. Detailed hook documents are vendored progressively under `rules/`; references to `./references` in the upstream text refer to those files in this pinned package.
