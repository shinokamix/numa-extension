---
name: shadcn
description: Use for choosing, manually sourcing, adapting, styling, or composing shadcn/ui primitives in Numa; not for unrelated product-specific React composition.
...
This profile takes precedence over every upstream CLI or project-context instruction below. Read `../../../DESIGN.md`; it is Numa's authoritative design policy. Numa manually copies only the required shadcn/ui source into its current FSD owner and records the pinned source URL plus numbered adaptations. Product-specific composition remains local and need not originate from shadcn/ui. Do not run shadcn initialization, installation, add, apply, generator, MCP, package-install, or overwrite commands unless the user explicitly requests that exact operation; the upstream command-oriented sections are reference material only. Do not create `components.json`. Preserve Numa's semantic tokens, accessibility, formatting, and lint rules, then run the checks required by the root `AGENTS.md`.
